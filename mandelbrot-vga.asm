...
escaped:
    ...
    mov bl, cl  ; uSE Iteration as the color (0-15)
    and bl, 0x0f    ; Ensure its the 4-bit color
    or bl, 0x08     ; Use "bright" colors
...
; ---------------- BIOS output with color ----------------
put_space:
    mov al, ' '
    mov bl, 0x07        ; Normal white on black for spaces
    jmp putc_color

putc:
    mov bl, 0x07        ; Default color for CRLF etc.

putc_color:
    ; AH=09h: write char+attribute at cursor, BL=attribute, CX=count
    mov ah, 0x09
    xor bh, bh          ; Page 0
    ...
    ; Advance cursor manually (AH=02h)
    inc dl
    mov ah, 0x02
    xor bh, bh
    int 0x10
...
; ---------------- CRLF (still uses 0Eh, color doesn't matter) ----------------
putcrlf:
    mov ah, 0x0E
    mov al, 0x0D
    xor bx, bx
